...
class Ripper:
    def __init__(
        self,
        device: str = "/dev/sr0",
        cdparanoia_path: str = "cdparanoia",
        flac_path: str = "flac",
    ...
    def rip_cd(self, tracks: list[dict]) -> None:
        ...
        meta0 = tracks[0]
        artist_dir = sanitize_filename(
            meta0.get("ALBUM_ARTIST", meta0.get("ARTIST", "Unknown Artist"))
        )
        ...
        print("=" * 60)
        print(
            f"🚀 Rip and encode of {len(tracks)} tracks from {self.device} starting...\n"
        )
        ...
        with tempfile.TemporaryDirectory(prefix="cd_rip_") as tmpdir:
            for meta in tracks:
                ...
                if not self.rip_track(num, wav_p):
                    print_error(f"Failed to rip track {num:02d}")
                    errors += 1
                    continue
                ...
                if self.encode_flac(wav_p, flac_p, meta):
                    ...
                    print_error(f"Failed to encode track {num:02d}")
                    errors += 1
