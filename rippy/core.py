...
def _process_track_list(release, track_list, medium, tracks_len) -> list[dict]:
    ...
    for track in track_list:
        ...
        meta = {
            ...
            "MUSICBRAINZ_RELEASEGROUPID": release.get("release-group", {}).get("id"),
        ...
        if medium.get("position"):
            meta["DISCNUMBER"] = medium.get("position")
...
def extract_tracks(release: dict, disc=None) -> list[dict]:
    """Extracts track metadata from a MusicBrainz release.

    If a disc is provided, filters for tracks from the correct disk (in multi-disk scenarios).
    """

    tracks = []
    # For multi-disk releases, filter by the disc ID of the inserted CD
    if disc and "medium-list" in release:
        # Try to find the specific medium that matches our disc
        for medium in release["medium-list"]:
            # Check if this medium has a disc ID that matches our inserted disc
            if "disc-list" in medium:
                for disc_info in medium["disc-list"]:
                    if disc_info.get("id") == disc.id:
                        # This is the medium we're looking for
                        track_list = medium.get("track-list", [])
                        tracks_len = len(track_list)
                        tracks.extend(
                            _process_track_list(release, track_list, medium, tracks_len)
                        )
                        break
            else:
                # If no disc-list is found, use the first medium
                track_list = medium.get("track-list", [])
                tracks_len = len(track_list)
                tracks.extend(
                    _process_track_list(release, track_list, medium, tracks_len)
        ...
        # If no disc specified or no medium list, process all tracks normally
        for medium in release.get("medium-list", []):
            track_list = medium.get("track-list", [])
            tracks_len = len(track_list)
            tracks.extend(_process_track_list(release, track_list, medium, tracks_len))
