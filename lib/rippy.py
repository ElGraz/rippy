...
def print_progress(stream: IO[AnyStr] | None):
    if stream is None:
        return
...
def rip_track(track_num: int, wav_path: Path, device: str = "/dev/sr0") -> bool:
    """
    ...
    cmd = [
        ...
        "-d",
        device,
        str(track_num),  # Numero traccia da estrarre
